...
| **BGP Health Check**|                        |                                                                       | [Details](/docs/usage/bgp-health-check/)                                        |
|                     | `--controlPlaneHealthCheckAddress` | URL to poll (e.g. `https://localhost:6443/livez`)       | Empty disables the health check                                                 |
|                     | `--controlPlaneHealthCheckPeriodSeconds` | default 5                                          | Seconds between health checks                                                   |
|                     | `--controlPlaneHealthCheckTimeoutSeconds` | default 3                                         | Timeout per health check request                                                |
|                     | `--controlPlaneHealthCheckFailureThreshold` | default 3                                      | Consecutive failures before BGP route withdrawal                                |
|                     | `--controlPlaneHealthCheckCAPath` | Path to CA cert for HTTPS verification                     | Required when using HTTPS with a private CA                                     |
...
|                     | `control_plane_health_check_period_seconds` | default 5                                                     | Seconds between health checks                                                   |
|                     | `control_plane_health_check_timeout_seconds` | default 3                                                    | Timeout per health check request                                                |
